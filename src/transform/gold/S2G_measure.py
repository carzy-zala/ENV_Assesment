...
def create_fact_measurement(conn: sqlite3.Connection) -> None:
    conn.execute("""
    ...
    conn.execute("""
        ...
        ON fact_measurement(station_id, datetime)
    """)
    conn.execute("""
        CREATE INDEX IF NOT EXISTS idx_fact_param_unit
        ON fact_measurement(parameter, unit)
    """)
    conn.commit()


def insert_fact_measurement_from_silver(conn: sqlite3.Connection, ingested_at: str) -> int:
    """
    Insert measurements for this batch, joined to dim_station to get station_key.
